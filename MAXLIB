-- MAX-Q UI Library (Expanded & Feature Rich)
local Library = {}
...
local function tween(obj, props, time) local tw = TweenService:Create(obj, TweenInfo.new(time or 0.2, Enum.EasingStyle.Exponential), props) tw:Play() return tw end
...
if guiParent:FindFirstChild("MaxQ-Library") then
    guiParent["MaxQ-Library"]:Destroy()
end

local screenGui = create("ScreenGui", {
    Name = "MaxQ-Library", ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling, IgnoreGuiInset = true, Parent = guiParent
})
...
function Library:Notify(title, message, duration)
    ...
    tween(scale, {Scale = 1}, 0.3) tween(toast, {BackgroundTransparency = 0}, 0.3) tween(stroke, {Transparency = 0}, 0.3) tween(titleLbl, {TextTransparency = 0}, 0.3) tween(msgLbl, {TextTransparency = 0}, 0.3)
    
    task.delay(duration, function()
        if not toast.Parent then return end
        tween(scale, {Scale = 0.8}, 0.3) tween(toast, {BackgroundTransparency = 1}, 0.3) tween(stroke, {Transparency = 1}, 0.3) tween(titleLbl, {TextTransparency = 1}, 0.3) tween(msgLbl, {TextTransparency = 1}, 0.3)
...
local function BuildElements(parentContainer)
    ...
    function Elements:CreateSection(title)
        ...
        addCorner(sf, 4) addStroke(sf, Theme.Border, 1)
        ...
        local function upd()
            if open then
                icon.Text = "-"
                tween(sf, {Size = UDim2.new(1, 0, 0, 36 + cont.AbsoluteSize.Y)}, 0.2)
            else
                icon.Text = "+"
                tween(sf, {Size = UDim2.new(1, 0, 0, 36)}, 0.2)
            end
        end
    ...
    function Elements:CreateToggle(title, default, callback)
        ...
        addCorner(tf, 4) addStroke(tf, Theme.Border, 1)
        ...
        local function updateState(newState)
            toggled = newState
            tween(btn, {BackgroundColor3 = toggled and Theme.Success or Theme.Tertiary})
            tween(circ, {Position = toggled and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7)})
    ...
    function Elements:CreateButton(title, callback)
        ...
        btn.MouseEnter:Connect(function() tween(bf, {BackgroundColor3 = Theme.Tertiary}) end)
        btn.MouseLeave:Connect(function() tween(bf, {BackgroundColor3 = Theme.Secondary}) end)
        btn.MouseButton1Click:Connect(function()
            local s = create("UIScale", {Scale=0.95, Parent=bf}) tween(s, {Scale=1}, 0.15) task.delay(0.15, function() s:Destroy() end)
            if callback then callback() end 
        end)
    end

    function Elements:CreateDropdown(title, optionsList, default, callback)
        ...
        addCorner(df, 4) addStroke(df, Theme.Border, 1)
        ...
        local function toggleDrop()
            open = not open
            tween(df, {Size = open and UDim2.new(1, 0, 0, 140) or UDim2.new(1, 0, 0, 36)}, 0.2)
            tween(listFrame, {Size = open and UDim2.new(1, -24, 0, 90) or UDim2.new(1, -24, 0, 0)}, 0.2)
        end
        ...
        local function populate(opts)
            ...
            for _, opt in ipairs(opts) do
                ...
                addCorner(optBtn, 4)
                optBtn.MouseEnter:Connect(function() tween(optBtn, {BackgroundColor3 = Theme.Tertiary}) end)
                optBtn.MouseLeave:Connect(function() tween(optBtn, {BackgroundColor3 = Theme.Background}) end)
                optBtn.MouseButton1Click:Connect(function()
                    selected = opt dropBtn.Text = tostring(opt) toggleDrop()
    ...
    function Elements:CreateMultiDropdown(title, optionsList, defaultList, callback)
        local selected = {}
        ...
        local function getSelectedStr()
            local t = {} for k, v in pairs(selected) do if v then table.insert(t, tostring(k)) end end
            if #t == 0 then return "None" end return table.concat(t, ", ")
        end
        ...
        local function toggleDrop()
            open = not open
            tween(df, {Size = open and UDim2.new(1, 0, 0, 160) or UDim2.new(1, 0, 0, 36)}, 0.2)
            tween(listFrame, {Size = open and UDim2.new(1, -24, 0, 110) or UDim2.new(1, -24, 0, 0)}, 0.2)
        end
        dropBtn.MouseButton1Click:Connect(toggleDrop)
        ...
        local function populate(opts)
            ...
            for _, opt in ipairs(opts) do
                local optF = create("Frame", { Size = UDim2.new(1, 0, 0, 24), BackgroundColor3 = Theme.Background, Parent = listFrame })
                addCorner(optF, 4)
                ...
                local function upd()
                    box.BackgroundColor3 = selected[opt] and Theme.Success or Theme.Tertiary
                    btn.TextColor3 = selected[opt] and Theme.Text or Theme.TextDark
                end
                upd()
    ...
    function Elements:CreateSearchableDropdown(title, optionsList, default, callback)
        local selected = default or (optionsList and optionsList[1]) or ""
        local df = create("Frame", { Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = Theme.Secondary, Parent = parentContainer, ClipsDescendants = true })
        addCorner(df, 4) addStroke(df, Theme.Border, 1)
        ...
        local function toggleDrop()
            open = not open
            tween(df, {Size = open and UDim2.new(1, 0, 0, 180) or UDim2.new(1, 0, 0, 36)}, 0.2)
            tween(listFrame, {Size = open and UDim2.new(1, -24, 0, 98) or UDim2.new(1, -24, 0, 0)}, 0.2)
        ...
        searchBox:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            for _, v in ipairs(optBtns) do
                if q == "" or string.find(v.txt, q) then
                    v.btn.Visible = true
                else
                    v.btn.Visible = false
                end
    ...
    function Elements:CreateKeybind(title, defaultKey, callback)
        ...
        return { Set = function(self, key) currentKey = key keyBtn.Text = currentKey.Name end, Value = function() return currentKey end }
    end

    function Elements:CreateColorPicker(title, defaultColor, callback)
        ...
        addCorner(cf, 4) addStroke(cf, Theme.Border, 1)
        ...
        colorBtn.MouseButton1Click:Connect(function()
            open = not open
            tween(cf, {Size = open and UDim2.new(1, 0, 0, 80) or UDim2.new(1, 0, 0, 36)}, 0.2)
            tween(palette, {Size = open and UDim2.new(1, -24, 0, 30) or UDim2.new(1, -24, 0, 0)}, 0.2)
        end)
    ...
    function Elements:CreateDivider(text)
        ...
        if text then
            create("TextLabel", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Theme.TextDark, TextSize = 11, Font = Enum.Font.GothamMedium, TextXAlignment = Enum.TextXAlignment.Left, Parent = dv })
        else
            create("Frame", { Size = UDim2.new(1, -24, 0, 1), Position = UDim2.new(0, 12, 0.5, 0), BackgroundColor3 = Theme.Border, BorderSizePixel = 0, Parent = dv })
        end
    end
...
function Library:CreateWindow(options)
    ...
    local closeBtn = create("TextButton", { Size = UDim2.new(0, 24, 0, 24), Position = UDim2.new(1, -8, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundTransparency = 1, Text = "X", TextColor3 = Theme.TextDark, TextSize = 14, Font = Enum.Font.GothamMedium, Parent = titleBar })
    ...
    function Window:CreateTab(name)
        ...
        tabBtn.MouseButton1Click:Connect(function()
            if activeTabBtn then tween(activeTabBtn, {BackgroundTransparency = 1, TextColor3 = Theme.TextDark}) activePage.Visible = false end
            activeTabBtn = tabBtn activePage = page tween(tabBtn, {BackgroundTransparency = 0, TextColor3 = Theme.Text}) page.Visible = true
        end)
